    ...
    if request.method == 'POST':
        ...
        if relevant_personnel == 'True':
            ...
            if originator_relevant:
                relevant_personnel_obj=Task_Relevant_Personnel(u_type="Originator",u_id=user_obj['data']['u_aid'],
        ...
        if task_summery_check == 'True':
            task_summery=request_data['task_summery_comments']
            task_requirement=request_data['task_requirements_comments']
            assignee_comments=request_data['task_assignee_comments']
            # approval=request_data['approval']
            approver_comments=request_data['task_approver_comments']
...
def issue_registrartion(request):
    ...
    if request.method == 'POST':
        ...
        if issue_summery_check == 'True':
            issue_summery=request_data['issue_summery_comments']
            issue_requirement=request_data['issue_requirements_comments']
            issue_assignee_comments=request_data['issue_assignee_comments']
            # issue_approval=request_data['issue_approval']
            issue_approver_comments=request_data['issue_approver_comments']
            ...
            issue_summery_obj=IssueSummery(issue_summery=issue_summery,issue_requirement=issue_requirement,assignee_comments=issue_assignee_comments,
                                                     ...
                                                     issue=issue_registration_obj,addedby=added_by,adddate=today_date)
            issue_summery_obj.save() 
        return redirect('issue_registrartion')
...
def task_approver(request):
    ...
    task_obj=TaskRegistration.objects.get(task_id=taskid)
    originator=task_obj.originator

    users_obj=Users.objects.all()

    assignee_user=list()
    relevant_personal_assignee=Task_Relevant_Personnel.objects.filter(task=taskid,u_type='Assignee')
    ...
    approver_user=list()    
    relevant_personal_approver=Task_Relevant_Personnel.objects.filter(task=taskid,u_type='Approver')
    ...
    # task_relevant_obj=Task_Relevant_Personnel.objects.filter(u_id=U_id,u_type="Approver")
    context={"task_relevant_obj":api_data['task_relevant_obj'],'task_approval_master':api_data['task_approval_master'],'taskid':taskid,
            'assignee_user_obj':assignee_user_lst,
            'originator_name':originator,'approver_user_obj':approver_user_lst}
    return render(request,'task_approver.html',context)
...
#     api_data=response.json()
#     print('api_data ')
#     context={"task_relevant_obj":api_data['task_relevant_obj'],"taskid":taskid}
...
def task_assignee(request):
    ...
    api_data=response.json()
    print('api_data ')
    context={"task_relevant_obj":api_data['task_relevant_obj'],"taskid":taskid,'assignee_user_obj':assignee_user_lst,
            'originator_name':originator,'approver_user_obj':approver_user_lst}
    return render(request,'task_assignee.html',context)
...
def update_summery_data(request):
    ...
    if approval:
        ...
        task_registration_obj.save()
        objmaster.insertActivityTrail(task_id,"8","Task updated by approver - "+task_registration_obj.task_name,request.session['uid'],request.session['accessToken'])
    if assignee_comments: 
        ...
        task_registration_obj.completion_status=completion_status
        task_registration_obj.save() 
        objmaster.insertActivityTrail(task_id,"8","Task updated by assignee - "+task_registration_obj.task_name,request.session['uid'],request.session['accessToken'])
...
def issue_approver(request): 
    ...
    context={"issue_relevant_obj":api_data['issue_relevant_obj'],'issueid':api_data['issueid'],'originator':originator,
...
def issue_assignee(request): 
    ...
    api_data=response.json() 
    print("api_data",api_data)
    context={"issue_relevant_obj":api_data['issue_relevant_obj'],'issueid':issueid,'originator':originator,
            'assignee_user_obj':assignee_user_lst,'approver_user_obj':approver_user_lst}
    return render(request,'issue_assignee.html',context)
...
def issue_update_summery_data(request):
    ...
    if approval:
        ...
        issue_registration_obj.save()
        objmaster.insertActivityTrail(issue_id,"11","Issue updated by assignee", request.session['uid'],request.session['accessToken'])
    if assignee_comments:
