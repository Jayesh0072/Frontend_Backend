...
def dataCleaning(request):
    try:
        ...
        idx=0
        for key, value in dttypes.items():
            gridDttypes.append({'colName': key, 'dataType': value,'idx':str(idx)})
            idx=idx+1
        _isDisabled=""
...
def showSNSChart(request):
    try: 
        ...
        plt.tight_layout()
        #plt.show()
        # sns_plot.savefig(os.path.join( BASE_DIR, 'static\\media\\'+user_name+'output.png'))
        ...
        if(content == "NoData"):
            if(os.path.exists(savefile_x_keep)):
                ...
                processing = os.path.join(
                    BASE_DIR, 'static\\reportTemplates\\processing.csv')
                df_old_proc = pd.read_csv(processing)
    ...
    try:
        ...
        if os.path.exists(savefile_x_scaled):
            ...
            if not(targetVar=="None"):
                ...
                print('x_scaled_df.columns ',len(x_scaled_df.columns))
    ...
    try:
        ...
        for colval in json_colDataLst:
            for attribute, value in colval.items():
                print(attribute, value)
                if(value != ""):
                    if(df[content].dtypes=='float64'):
                        colLst.append(float(attribute))
                        valLst.append(float(value))
                    else:
                        colLst.append(attribute)
                        valLst.append(value)
                    updatedDataLst.append([content,attribute,value])
        df[content].replace(colLst, valLst, inplace=True)
        print('content ',content,df[content].dtypes)
        print('colLst ',colLst)
        print('valLst ',valLst)
        savefile_x = file_path + file_name + "_x.csv"
        df.to_csv(savefile_x, index=False)
    ...
    try:
        ...
        if os.path.exists(cnfrmsrcFiles):
            ...
            for key, value in dttypes.items():
                ...
                srcLst.append(objlstCnfrmSrc)
                maxid += 1
        return render(request, 'confirmSource3.html', {'txtList': srcLst, 'emailLst': getEmails(),'occpae32' : "/static/reportTemplates/pub-ch-model-risk.pdf#page=32&zoom=100,0,400"})
    except Exception as e:
        print(e)
        return render(request, 'error.html')
                    ...
                    for index, row in dffilter.iterrows():
                        ...
                        if os.path.exists(os.path.join(
                            ...
                            if(row["chartType"] == "Heatmap"):
                                saveHeatmapImage()
                                pdf = exportgraphImgPdf(pdf, os.path.join(
                                    BASE_DIR, 'static\\media\\'+row["imageName"]),  "Correlation on independent variables-Heat map", comments)
                            else:
                                pdf = exportgraphImgPdf(pdf, os.path.join(
                                    BASE_DIR, 'static\\media\\'+row["imageName"]), row["chartType"]+" "+row["xaxisval"]+" vs "+row["yaxisval"], comments)
                            pdf.output(os.path.join(
...
def TestHIST(request):
    try: 
        ...
        # dfvx = vx.from_csv(savefile_x_keep, convert=True) 
        # x_keep=dfvx.to_pandas_df() 
        # del dfvx
        plt.style.use('fivethirtyeight')
        ...
        if(chartType=="Data"):
            ...
            print('x_keep[selCol] ',x_keep[selCol].value_counts())
...
class CsvApiView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        try:
            csv_file_path = os.path.join(settings.BASE_DIR, 'app1/static/data.csv')

            # Initialize an empty DataFrame
            df = pd.DataFrame()

            # Read existing CSV data using Pandas
            if os.path.exists(csv_file_path):
                try:
                    df = pd.read_csv(csv_file_path) 
                    df.replace([np.inf, -np.inf, np.nan], 0, inplace=True)
                except pd.errors.EmptyDataError:
                    return Response({'error': 'CSV file is empty or improperly formatted'}, status=status.HTTP_400_BAD_REQUEST)
                except Exception as e:
                    return Response({'error': f'Error reading CSV file: {e}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        except Exception as e:
    ...
    def post(self, request, *args, **kwargs):
        try:
            if 'myfile' in request.FILES:
                ...
                if os.path.getsize(uploaded_file_path) > 0:
                    try:
                        new_df = pd.read_csv(uploaded_file_path)
                        
                        # Check if the new DataFrame has columns to avoid EmptyDataError
                        if not new_df.empty and not new_df.columns.empty:
                            ...
                            insert_data_to_sql(new_df,request)
                        else:
                            return Response({'error': 'Uploaded CSV has no data'}, status=status.HTTP_400_BAD_REQUEST)
                    except pd.errors.EmptyDataError:
                        return Response({'error': 'Uploaded file is empty or improperly formatted'}, status=status.HTTP_400_BAD_REQUEST)
                    except Exception as e:
...
def insert_data_to_sql(df,request):
    print("df--------------",df)
    ...
    #     for index, row in df.iterrows():
    #         # Modify this que]ry to match your table schema
    #         insert_query = """
    #         INSERT INTO xyz (Name, Age, Departement)
    #         VALUES (?, ?, ?)
    #         """
    #         # Execute the query with the row data
    #         cursor.execute(insert_query, row['Name'], row['Age'], row['Departement'])
    ...
    #     conn.close()
    # except Exception as e:
    #     print("error is mapping-------------------",e)
    #     print('traceback is ', traceback.print_exc())
    #     return Response({'error': 'Error inserting data into SQL Server'}, status=status.HTTP_400_BAD_REQUEST)
