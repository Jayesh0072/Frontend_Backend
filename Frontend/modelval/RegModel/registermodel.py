...
class RegisterModel:
    ...
    def getUserDeatils(self,utype,dept):
        strq= "select rs.r_aid,r_label, 'block' sts  ,UA_Perm 'AccessPerm' from user_access ua , Resources rs where rs.R_AID=ua.r_AID and  ua.uc_AID='"+str(utype)+"' "
        strq=strq + " union "
    ...
    def getMdlUpstrem(self,uid,mdl_id=""):
        ...
        if mdl_id != "":
            tableResult =self.objdbops.getTable("select mdl_id from Mdl_OverView where mdl_id <> '"+mdl_id+"' and AddedBy="+str(uid))
            Reliance= tableResult.to_json(orient='index')
            return json.loads(Reliance)
        else:
    ...
    def updateMdlVersion(self,mdlId,isMinor):
        ...
        mdlversion=''
        if isMinor =="1":
            strQ="select  concat ( case when  (is_tool)=0 then 'M' else 'T' end ,  format(max(mdl_cnt),'00'), \
                format(max(mdl_major_ver),'00') , format(max(mdl_minor_ver)+1,'00')) \
                from Mdl_OverView where substring(Mdl_Id,1,3)='"+mdlId[:3]+"' group by is_tool"
            
            newmdlid=self.objdbops.getscalar(strQ)    
            strQ="select  concat (  format(max(mdl_cnt),'00'),'-', \
                format(max(mdl_major_ver),'00') , '-',format(max(mdl_minor_ver)+1,'00')) \
                from Mdl_OverView where substring(Mdl_Id,1,3)='"+mdlId[:3]+"' group by is_tool"
            mdlversion=self.objdbops.getscalar(strQ)        
        else:
            strQ="select  concat ( case when  (is_tool)=0 then 'M' else 'T' end ,  format(max(mdl_cnt),'00'), \
                format(max(mdl_major_ver)+1,'00') , format(max(mdl_minor_ver),'00')) \
                from Mdl_OverView where substring(Mdl_Id,1,3)='"+mdlId[:3]+"' group by is_tool"
            newmdlid=self.objdbops.getscalar(strQ)
    ...
    def getModelListByUSerid(self,userId,userLvl,istool): 
        ...
        if str(objmaster.checkMRMHead(str(userId)))=="1":
            ...
            strQ+= " group by mdl_id  ) VTdata on VTdata.mdl_id=mdl_overview.Mdl_Id where isnull(Is_Decommissioned,0) =0"
            strQ+=" order by mdl_overview.adddate"   
        else:
            strQ="select isnull(str(vtcnt),'') vtcnt,case   when  isnull(vtcnt,0) >0 then 'text-danger' else '' end Activity,isnull(Mdl_Risk_Label,'') Mdl_Risk_Label,isnull(Intrinsic_Label,'') Intrinsic_Label,"
            strQ+=" isnull(Reliance_Label,'') Reliance_Label,isnull(Materiality_Label,'') Materiality_Label, mdl_overview.*,"
    ...
    def getModelById(self,mdlId):
        ...
        #     strQ="select isnull(Mdl_Risk_Label,'') Mdl_Risk_Label,isnull(Intrinsic_Label,'') Intrinsic_Label,"
        #     strQ+=" isnull(Reliance_Label,'') Reliance_Label,isnull(Materiality_Label,'') Materiality_Label, mdl_overview.*,"
        #     strQ+=" isnull(mdl_risks.Risk_Mtgn,'') Risk_Mtgn,isnull(mdl_risks.Fair_Lndg,'') Fair_Lndg ,format(mdl_overview.adddate,'MM/dd/yyyy') regdate,Dept_Label from("
        #     strQ+=" select Mdl_Src_Label,Mdl_Use_Freq_Label,Prd_Addr_Label,Mdl_Fncn_Label,mdl_oview.*,Mdl_Type_Label from  "
        #     strQ+=" Mdl_OverView mdl_oview,Model_Type_Master mdl_ty_mstr ,"
        #     strQ+=" Model_Source_Master mdl_src_mst ,Model_Use_Freq_Master mdl_use_freq_mst ,Prd_Addr_Master prd_addr_mst,Model_Function_Master funct_mst"
        #     strQ+=" where mdl_oview.addedby="+ str(userId)
        #     strQ+=" and mdl_oview.mdl_type=mdl_ty_mstr.Mdl_Type_AID"
        #     strQ+=" and Mdl_Scr_AID=Mdl_Source"
        #     strQ+=" and mdl_use_freq_mst.Mdl_Use_Freq_AID=usgFreq"
        #     strQ+=" and prd_addr_mst.Prd_Addr_AID=prctAddr and funct_mst.Mdl_Fncn_AID=func and mdl_oview.is_tool=0 )mdl_overview left join"
        #     strQ+=" ("
        #     strQ+=" select isnull(Mdl_Risk_Label,'') Mdl_Risk_Label,isnull(Intrinsic_Label,'') Intrinsic_Label,isnull(Reliance_Label,'') Reliance_Label,isnull(Materiality_Label,'') Materiality_Label,mdl_risks.* from  Mdl_Risks mdl_risks left join  Mdl_Risk_Master mdl_risks_mst  "
        #     strQ+=" on Mdl_Risk_AID=mdl_risks left join Intrinsic_Master on Intrinsic_AID=Intr_Risk" 
        #     strQ+=" left join Reliance_Master on Reliance_AID=Reliance   left join Materiality_Master on Materiality_AID =Materiality"
        #     strQ+=" ) mdl_risks"
        #     strQ+=" on mdl_overview.Mdl_Id =mdl_risks.Mdl_Id , users ,Department dept  where mdl_overview.mdl_id='"+ mdlId +"'  and mdl_overview.AddedBy=U_AID and users.Dept_AID=dept.Dept_AID"
        #     strQ+=" order by mdl_overview.adddate"
        # elif userLvl=="2":
        strQ="select isnull(Mdl_Risk_Label,'') Mdl_Risk_Label,isnull(Intrinsic_Label,'') Intrinsic_Label,"
        strQ+=" isnull(Reliance_Label,'') Reliance_Label,isnull(Materiality_Label,'') Materiality_Label, mdl_overview.*,"
...
class MdlOverviewCls:
    ...
    def __init__(self,Mdl_Id,Mdl_Category,Mdl_Sub_Category,Mdl_Cnt ,Mdl_Major_Ver , Mdl_Minor_Ver ,isnew ,is_tool ,Department
                 ,Func,Prm_Name, Sec_Name,Mdl_Source,Mdl_Type,Mdl_Absct,Mdl_objective,Mdl_Appl,
                 Mdl_Risk_Anls,PrctAddr,UsgFreq,AddedBy,txtPrdDt):
        self.objdbops =dbops()
        self.Mdl_Id=Mdl_Id
        self.Mdl_Category=Mdl_Category
        self.Mdl_Sub_Category=Mdl_Sub_Category
    ...
    def insertMdlOverview(self):       
        ...
        if(self.isnew=="1"):
            ...
            newcnt=self.Mdl_Cnt
        print(str(self.Mdl_Major_Ver),',',str(self.Mdl_Minor_Ver))

        strQ="insert into Mdl_OverView ( Mdl_Id "
        strQ+=" , Mdl_Cnt "
        strQ+=" , Mdl_Category "
        strQ+=" , Mdl_Sub_Category "
        strQ+=" , Mdl_Major_Ver "
        strQ+=" , Mdl_Minor_Ver "
        strQ+=" , isnewupdate "
        strQ+=" , is_tool "
        ...
        strQ+=" '"+ self.Mdl_Id +"'"
        strQ+=" ,  "+str(newcnt)
        strQ+=" ,'"+ self.Mdl_Category +"'"
        strQ+=" ,'"+ self.Mdl_Sub_Category +"'"        
