...
def modelList(request):
    try:
        ...
        # return render(request, 'addICQQtns.html',{'sections':objmaster.getSections(),'actPage':'RMSE','notifylen':str(len(objvalidation.getVTNotifications(request.session['uid'])))})
        return render(request, 'modelList.html',{'actPage':'RMSE','modelinfo':modelinfo,'canAdd':canAdd})
    except Exception as e:
        print('setuppycaret is ',e)
        print('setuppycaret traceback is ', traceback.print_exc()) 
...
def modelDetails(request):
    try:
        ...
        return render(request, 'modelDeatails.html',{'actPage':'RMSE','mdl_id':mdlId,'task_list':task_list,'mdlsts':api_data['mdlsts'],
                        'issue_list':api_data['issue_list'],'validationRatings':api_data['validationRatings'],'modelDocs':objvalidation.getModelDocs(mdlId),'activity_lst':activity_lst})
    except Exception as e:
        print('setuppycaret is ',e)
        print('setuppycaret traceback is ', traceback.print_exc()) 
...
def addnewuser(request):
    try: 
        ...
        return render(request, 'adduser.html',{ 'actPage':'Add User','DisplayBackUp':DisplayBackUp,'backupfor':backupfor,'u_depart_name':u_depart_name,
                                               ...
                                               'IfBackupUser':IfBackupUser,'BackUpFor_UID':BackUpFor_UID})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc()) 
...
def addUser(request):
    try:
        ...
        if update_id:
            ...
            strQ="INSERT INTO Users (U_Name  ,U_Email ,U_FName ,U_LName  ,U_ProfilePic  ,UC_AID  ,U_Description ,ActiveStatus,AddedBy ,AddDate,Dept_AID,U_reportto"
            if(isBackUp=="1"):
                strQ+=",U_AID_BackUpFor,BackUp_AID,StartDate,EndDate"
            strQ+=") "
            strQ += " VALUES  ('"+ uname+"','"+ email+"','"+ fname+"','"+ lname+"',null,'"+ utype+"',null,'"+ activests+"', '"+str(request.session['uid'])+"',getdate(),"+ dept_aid +","+ reportsto +""
            if(isBackUp=="1"):
                strQ+=",'"+ U_AID_BackUpFor+"','"+ AID_BackUp+"','"+ StartDate+"','"+ EndDate+"'"
            strQ+=")" 
            objdbops.insertRow(strQ)
            objdbops.insertRow(" update users set U_Password=convert(varbinary(max), ENCRYPTBYPASSPHRASE(concat(U_Name,'1'), U_Name)) where u_name='"+ uname+"'")
...
def dashboard(request):
    try:  
        ...
        if dttbl.empty == False:
            ...
            request.session['li_qtns']=dttbl[dttbl['r_label'] =='Questions'].values[0][2]  
            request.session['li_vrs']=dttbl[dttbl['r_label'] =='Validation Report Submission'].values[0][2]  
...
def issueLst(request):
    try: 
        ...
        api_data=response.json() 
        if(str(objmaster.isAutherized(request.session['ucaid'],"Quick View")) =="0"): 
            return render(request, 'blank.html',{'actPage':'RMSE'})  
...
def activitytrail(request):
    try: 
        ...
        return render(request, 'activitytrail.html',{'actPage':'RMSE - Activity Trail','activity_lst':activity_lst,'mdlid_lst':mdlid_lst,'refer_id':refer_id})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())
...
def getMdlDetailsById(request):
    try:
        ...
        return JsonResponse({'istaken':'true','dependencies':dependencies,'PerformMon':PerformMon,'Owner':Owner,
...
def activate_virtualenv(env_path,mdl_id):
    # print("MDL id---------------",mdl_id)
    # Replace 'venv' with the name of your virtual environment
    # virtualenv_path = os.path.join(os.getcwd(), env_test)
    env_dir=mdl_id+'_Venv'
    virtualenv_path = os.path.join(os.getcwd(),'static\\document_files\\'+mdl_id+'\\', env_dir)
    print("virtualenv_path",virtualenv_path)
    # Construct the activation command based on the operating system
    print("os.name",os.name)
    activate_command = 'activate' if os.name == 'nt' else 'source activate'

    # Form the full activation command
    full_command = os.path.join(virtualenv_path, 'Scripts', activate_command) if os.name == 'nt' else f'source {virtualenv_path}/bin/activate'
    print("full_command",full_command)
    # Execute the activation command using subprocess
    result=subprocess.run(full_command, shell=True)
    print("result env activation successfull",result)
    print("result.stdout",result.stdout)
    print("result.stderr",result.stderr)
    
    
import subprocess
def run_command(mdl_id):
    try:
        
        output=''
            
        command = 'virtualenv'+' '+mdl_id+'_Venv'
        # check_env = request.GET.get('check_env','False')
        # print("check_env",check_env)
        print("command",command[11:])

        ## Installing and creating virtual environment 
        # result_check_env = subprocess.run(check_env, shell=True, capture_output=True, text=True)
        # output_check_env = result_check_env.stdout + result_check_env.stderr
        # print("output_check_env",output_check_env)
        
        result = subprocess.run(command,cwd=os.path.join(os.getcwd(),'static\\document_files\\'+mdl_id+'\\'), shell=True, capture_output=True, text=True)
        output = result.stdout + result.stderr
        print("output",output)

        ### activating virtual environment
        env_test=command[11:]
        activate_virtualenv(env_test,mdl_id)
        data = {"msg":"Virtual Environment Created Successfull","status":"True"}
        return JsonResponse(data)
        # return render(request, 'create_env.html',{'output': output})
    except Exception as e:
        data = {"msg":e,"status":"False"}
        return JsonResponse(data)

import sys
def install_requirements(request):
    mdl_id = request.GET.get('mdl_id','False')
    requirements_file = os.path.join(BASE_DIR, 'static\\document_files\\'+mdl_id+'\\'+'requirements.txt')
    ...
    try:
        ...
        activate_script = os.path.join(virtualenv_path, 'Scripts', 'activate') if os.name == 'nt' else os.path.join(virtualenv_path, 'bin', 'activate')
        packages = 'C:/jayesh/my_packages/'

        if os.name == 'nt':  # Windows
            activate_cmd = f'{virtualenv_path}\\Scripts\\activate && pip install --no-index --find-links '+packages+' -r '+requirements_file

            print("activate_cmd",activate_cmd)
        else:  # Unix or MacOS
            activate_cmd = f'source {virtualenv_path}/bin/activate && pip install -r requirements.txt'
        ...
        #print("result.stdout",result.stdout)
        # print("result.stderr",result.stderr)
        # subprocess.check_call([sys.executable, full_command,'-m', 'pip', 'install','-r', requirements_file],cwd=os.path.join(BASE_DIR, 'static\\document_files\\'+mdl_id+'\\'+mdl_id+'_Venv'+'\\'))
        packages = 'C:/jayesh/my_packages/'
        # cwd=os.path.join(BASE_DIR, 'static\\document_files\\'+mdl_id+'\\'+mdl_id+'_Venv'+'\\')

        # from subprocess import Popen, PIPE
        # str_command=full_command+";pip install --no-index --find-links "+ packages +" -r "+requirements_file+";"

        # cmds=[full_command,"pip install --no-index --find-links "+ packages +" -r "+requirements_file]
        # from subprocess import Popen[p]

        # commands = [full_command, "pip install --no-index --find-links "+ packages +" -r "+requirements_file]
        # subprocess.Popen(commands)

        # subprocess.check_call([sys.executable,'-m', 'pip', 'install','--no-index --find-links',packages,'-r', requirements_file],cwd=os.path.join(BASE_DIR, 'static\\document_files\\'+mdl_id+'\\'+mdl_id+'_Venv'+'\\'))
        # processes = [Popen(cmd, shell=True) for cmd in commands] 
        # print('str_command ',str_command)
        # ret = subprocess.run(str_command, capture_output=True, shell=True)

        # # before Python 3.7:
        # # ret = subprocess.run(command, stdout=subprocess.PIPE, shell=True)

        # print("cmd output ",ret.stdout.decode())

        
    except subprocess.CalledProcessError as e:
        print(f"An error occurred while trying to install packages from {requirements_file}: {e}")
        return JsonResponse({"msg":"All packages installed successfully."},safe=False)
    else:
        print(f"All packages from {requirements_file} installed successfully.")
        return JsonResponse({"msg":"All packages installed successfully.","status":"True"},safe=False)
# install_requirements('M800100')
...
def addmodel(request):
    try:
        ...
        if request.method == 'POST': 
            # print("request_data",request.FILES.get('txtMdlData','False'))
            print("request_data_2",request.FILES.getlist('txtMdlData'))
            print("checkboxex1",request.POST.get('chkMdlData','False')) 
            ...
            fl_MD_chk = request.POST.get('chkMDD','False') 
            if fl_MD_chk == 'False':
                fl_MD = request.FILES.get('txtMDD', 'none')  
                print("devlopement files-------------",fl_MD)           
                if fl_MD != 'none':
                ...
                objdocs.inserDocs(mdl_id,'1','NA',str(request.session['uid']))

            fl_MD_chk = request.POST.get('chkUserManual','False') 
            if fl_MD_chk == 'False':
                ...
                objdocs.inserDocs(mdl_id,'2','NA',str(request.session['uid']))

            model_data_files = request.FILES.getlist('txtMdlData')
            # for i in model_data_files:
                
            for fl_MD in model_data_files:
                print("files----------",fl_MD)
                fl_MD_chk = request.POST.get('chkMdlData','False') 
                if fl_MD_chk == 'False':
                    ...
                    objdocs.inserDocs(mdl_id,'3','NA',str(request.session['uid']))

            fl_MD_chk = request.POST.get('chkMdlCode','False') 
            if fl_MD_chk == 'False':
                ...
                objdocs.inserDocs(mdl_id,'4','NA',str(request.session['uid']))
            
            fl_MD_chk = request.POST.get('chkUAT','False') 
            if fl_MD_chk == 'False':
                ...
                objdocs.inserDocs(mdl_id,'5','NA',str(request.session['uid']))
            ...
            if fl_MD_chk == 'False':
                ...
                objdocs.inserDocs(mdl_id,'6','NA',str(request.session['uid']))
            ...
            if fl_MD_chk == 'False':
                ...
                objdocs.inserDocs(mdl_id,'7','NA',str(request.session['uid']))

            # fl_MDl_code = request.FILES.get('txtMdlCode', 'none')
            # if fl_MDl_code != 'none':  
            #     fl_MD = request.FILES.get('txtRequirementFile', 'none')
            #     if fl_MD == 'none':
            #         print("Requirement File Mandatory")
            #     else:
            #         pass

            fl_MD_chk = request.POST.get('chkRequirementFile','False') 
            if fl_MD_chk == 'False':
                fl_MD = request.FILES.get('txtRequirementFile', 'none')            
                if fl_MD != 'none':
                    fs = FileSystemStorage()
                    ...
                    if os.path.exists(savefile_name):
                        os.remove(savefile_name)
                    fs.save(savefile_name, fl_MD)
                    objdocs.inserDocs(mdl_id,'8',fl_MD.name,str(request.session['uid']))
            else:
                objdocs.inserDocs(mdl_id,'8','NA',str(request.session['uid']))
                
            fl_MD_chk = request.POST.get('chkmergedMdlData','False') 
            if fl_MD_chk == 'False':
                fl_MD = request.FILES.get('txtmergedMdlData', 'none')             
                if fl_MD != 'none':
                    fs = FileSystemStorage()
                    savefile_name = destination_path + mdl_id+'_'+fl_MD.name
                    if os.path.exists(savefile_name):
                        os.remove(savefile_name)
                    fs.save(savefile_name, fl_MD)
                    objdocs.inserDocs(mdl_id,'9',mdl_id+'_'+fl_MD.name,str(request.session['uid']))
            else:
                objdocs.inserDocs(mdl_id,'9','NA',str(request.session['uid']))
...
def addtool(request):
    try:
        ...
        if request.method == 'POST':   
            ...
            # objPerformMon=MdlPerformanceMonitoring(mdl_id,txtApproach.replace("'","''"),ddlMonrFreq,txtMonrMtrcs.replace("'","''"),txtTgrEvt.replace("'","''")
            #                                        ,txtLstTgrDt,txtLstTgrMtgnDt,txtTgrEvtMtgn.replace("'","''"),request.session['uid'])
            # objPerformMon=MdlPerformanceMonitoring(mdl_id,request.session['uid'])
            # objPerformMon.insertPerfomanceMonitor()
...
def newdept(request):
    try:   
        ...
        return render(request, 'adddepartment.html',{ 'actPage':'Add Department','is_mrm_exists':ismrm})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())
...
def useraccess(request):
    try: 
        ...
        response = requests.get(third_party_api_url, headers=header)
        print("response department",json.loads(response.content))
        deptlist = json.loads(response.content)
        print("data department-----------------------",deptlist)
        return render(request, 'useraccess.html',{ 'actPage':'User Access','users':users,'utype':utype,'deptlist':deptlist})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())
...
def IsAuthenticate(utility,aid):
    print("----------1")
    try:
        obj_ro = UserAccess.objects.get(r_aid = aid)
        if obj_ro.ua_perm == 'ro':
            print("----------if")
            CanAdd = 'False'
        else:
            print("----------else")
...
# IsAuthenticate('PerfMontr',36,'ro')
...
def updateuseraccess(request):
    try:
        ...
        for colval in json_colDataLst:
            print('colval is ',colval['AID'],'UA',colval['UA'],colval['utility'],colval['add'],colval['edit'],colval['delete'])
            ...
            objdbops.insertRow(strQ)
            # IsAuthenticate(colval['utility'],colval['AID'],colval['UA'])
...
def getICQSecQtn(request):
    try:
        ...
        return JsonResponse({'sections':objmaster.getICQQtns(sectionid)})
    except Exception as e:
        print('reqValidation is ',e)
        print('reqValidation traceback is ', traceback.print_exc())
    ...
    try: 
        ...
        return render(request, 'ICQQtnsFinal.html',
                      {'ICQRating':api_data['ICQRating'],
                       'sectionid':sectionid,
                       'sections':api_data['sections'],'Qtns':api_data['Qtns'],})
    except Exception as e:
        print('reqValidation is ',e)
        print('reqValidation traceback is ', traceback.print_exc())
...
def FLQtnsFinal(request):
    try: 
        ...
        api_url_a=getAPIURL()+"InherentRiskRatingAPI/"       
...
def getFLSecQtnFinal(request):
    try:  
        api_url=getAPIURL()+"getFLSecQtnFinal/"       
...
def saveFLRatingsFinal(request):
    try:         
        ...
        uid=request.session['uid']
        api_url=getAPIURL()+"saveFLRatingsFinal/"       
        data_to_save={
            'colDataLst':colDataLst, 
            'uid':request.session['uid'], 
            } 
...
def pdfsummaryresp(request):  
    try:  
        ...
        api_data=response.json()
        print('Resp ',api_data['data'])
        return JsonResponse({"response":api_data['data'],'aiaudio':''})
    except Exception as e:
        print('setuppycaret is ',e)
        print('setuppycaret traceback is ', traceback.print_exc()) 
...
def edit_department(request,id): 
    try:
        ...
        return render(request, 'adddepartment.html',{ 'actPage':'Edit Department','label':label,'desc':desc,'is_mrm_exists':is_mrm,'id':id})
...
def rmse_calendar(request):    
    ...
    api_data=response.json() 
    context={'all_data_lst':api_data['all_data_lst']}    
...
def editregmodel(request): 
    try:
        ...
        print("Mdl_depndncy_obj",len(Mdl_depndncy_obj))    
        if len(Mdl_depndncy_obj) != 0:
            for Mdl_depndncy in Mdl_depndncy_obj:
                print("Mdl_depndncy.upstrmmdl",Mdl_depndncy.upstrmmdl)
        ...
        Dwstr_Model=objreg.getMdlUpstrem(request.session['uid'])
...
def updateregmodel(request):
    ...
    try: 
        ...
        data = {"mdl_id":request_data['mdl_id'],"isvalid":"true"}
        return JsonResponse(data)  
    except Exception as e:
        print('setuppycaret is ',e)
        print('setuppycaret traceback is ', traceback.print_exc())
...
def addModelMetrics(request):
    try: 
        ...
        response = requests.get(third_party_api_url, headers=header)
        print("response department",response.content) 
        return render(request, 'addModelMetrics.html',{ 'actPage':'Departments','users':json.loads(response.content)})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())
...
def PerfMontrSetupMRM(request):
    try:
        ...
        CanAdd = IsAuthenticate('Performance Monitoring',36)
        print("CanAdd-------------------",CanAdd)
        
        return render(request, 'PerfMontrSetup_MRM.html',{'data':data['mdlids'],'freqdata':data['frequency'],'ModelMatrics':data['mdlmetric'],'data_mdlids':data['data_mdlids'],'CanAdd':CanAdd})
    #'BusinessMetric':data['bussmetric'],
...
def gethistorymsg(request):
    try:
        ...
        return JsonResponse({'data':data['data'],'mmdata':data['mmdata']})
        # return JsonResponse(data['data'])
    except requests.exceptions.RequestException as e:
        return JsonResponse({'message': 'Failed to connect .', 'details': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def addReports(request):
    try: 
        ...
        # Section
        third_party_api_url = getAPIURL()+'ReportSectionMasterAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }    
        ...
        # Template name
        third_party_api_url = getAPIURL()+'get_template_name/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }    
        ...
        return render(request, 'addReports.html',{'departments':json.loads(responseget_department.content),'sections':json.loads(responseget_sections.content),'template_names':json.loads(responseget_tmpname.content)})
    except Exception as e:
        print('reqValidation is ',e)
        print('reqValidation traceback is ', traceback.print_exc()) 
...
def Save_Report_Sub_Section(request):
    try:
        third_party_api_url = getAPIURL()+'ReportSubSectionMasterAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
...
def getRptSub_Sections(request):
    try:       
        secid =request.GET.get('secid', 'False')  
        template_name = request.GET.get('template_name','False')
        third_party_api_url = getAPIURL()+'ReportSubSectionMasterAPI/'
...
def Save_Report_Sub_Sub_Section(request):
    try:
        third_party_api_url = getAPIURL()+'ReportSubSubSectionMasterAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
...
def getRptSub_Sub_Sections(request):
    try:       
        secid =request.GET.get('secid', 'False')  
        template_name = request.GET.get('template_name','False')
        third_party_api_url = getAPIURL()+'ReportSubSubSectionMasterAPI/'
...
def Save_Report(request):
    try:
        ...
        third_party_api_url = getAPIURL()+'ReportTemplateTempAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
...
def Submit_Report(request):
    try:
        third_party_api_url = getAPIURL()+'ReportTemplateAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
        ...
        if header_name != '':
            third_party_api_url_a = getAPIURL()+'RPT_Template_Header_API/'
            header = {
            "Content-Type":"application/json",
            'Authorization': 'Token '+request.session['accessToken']
            }    
...
@api_view(['POST'])
def addvalFindingdiscussion(request):
    try:
        print("request session",request.session.items())
        room_id = request.POST.get('room_id', 'False') 
        comment = request.POST.get('comment','False')
        finding_id = request.POST.get('finding_id','False')
        addedby = request.session['uid']
        # addedon = datetime.now()
        
        third_party_api_url = getAPIURL()+'ValFindingConnectionMsgSave/'
        data_to_save = {
            'room_id':room_id,
            'comment':comment,
...
@api_view(['GET'])
def ValFindgethistorymsg(request):
    try:
        room_id = request.GET.get('room_id', 'False') 
        findings_id = request.GET.get('findings_id', 'False')
        mdl_id = request.GET.get('mdl_id', 'False') 
        print("room_id",room_id,"mdl_id",mdl_id)
        third_party_api_url = getAPIURL()+'GetValFindHistoryMsg/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
...
def get_title_label(request):
    ...
    # return JsonResponse({'data':json.loads(responseget.content)})
...
def plotinsoccuvsincstate(request):
    try:
        ...
        context = {'chartType': context['chartType'], 'pdfFile': plot_dir_view+user_name+'Bar chart.pdf', 'graphpath': plot_dir_view+request.session['username']+'plotinsoccuvsincstate.png',
...
def plotinsoccuvsincstatestacked(request):
    try:
        ...
        context = {'chartType': context['chartType'], 'pdfFile': plot_dir_view+user_name+'Stacked Bar chart.pdf', 'graphpath': plot_dir_view+request.session['username']+'plotinsoccuvsincstatestacked.png', 
...
def stripplot(request):
    try:
        ...
        context = {'chartType': context['chartType'], 'pdfFile': plot_dir_view+user_name+'Strip Plot.pdf', 'graphpath': plot_dir_view+request.session['username']+'outputstripplot.png',
...
def vardistbyfraud(request):
    try:
        ...
        context = {'chartType': context['chartType'], 'pdfFile': plot_dir_view+user_name+'Distribution.pdf', 'graphpath': plot_dir_view+request.session['username']+'distbyfraud2.png',
...
def totalclaim_boxplot(request):
    try:
        ...
        context = {'chartType': context['chartType'], 'pdfFile': plot_dir_view+user_name+'Box Plot.pdf', 'graphpath': plot_dir+request.session['username']+'outputclaimboxplot.png',
...
def ValidationCommentHistory(request):
    try:
        ...
        print("utility",utility,"sub_utility",sub_utility,"chat_data",chat_data)
        ...
        data=responseget.json()
        print("responseget History Data--------------------------",data['data'])
        print("responseget History Data comments--------------------------",data['comment'])
...
def FL_addSub_Sub_Sub_Section(request):
    try:   
        sect_type=request.GET.get('stype', 'False')
        section =request.GET.get('section', 'False') 
        sectiondesc =request.GET.get('sectiondesc', 'False') 
        ...
        # objmaster.addSub_Sub_Sub_Section(sub_secid,section,sectiondesc,activests,str(request.session['uid']))
        api_url=getAPIURL()+"FL_addSub_Sub_Sub_Section/"  
...
def FL_add_question(request):
    try:
        ...
        section=request.GET.get('section','False')
        sub_section=request.GET.get('sub_section','False')
        sub_sub_section=request.GET.get('sub_sub_section','')
        # sub_sub_sub_section=request.GET.get('sub_sub_sub_section','False')
        question=request.GET.get('question','False')
        print("all data",sect_type,section,sub_section,question)
        api_url=getAPIURL()+"FL_add_question/"       
        params={'section':section,
            'sub_section':sub_section,
            'uid':added_by,
            'sub_sub_section':sub_sub_section,
...
def allocate_fl(request): 
    try:
        third_party_api_url = getAPIURL()+'Fl_sectionsAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
        ...
        third_party_api_url_user = getAPIURL()+'UsersgetAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
...
def save_flallocation(request):
    ...
    section_aid = request.GET.getlist('section_aid[]')
    users = request.GET.getlist('users[]')
    api_url=getAPIURL()+"save_flallocation/"       
...
def submitFLRatings(request):
    try: 
        # objreg.submitRatings( objmaster.getmaxICQId())
        # uid=request.session['uid']
        api_url=getAPIURL()+"submitFLRatings1/"       
        data_to_save={        
            'uid':request.session['uid'], } 
        header = {
        "Content-Type":"application/json",
...
def saveFLRatings(request):
    try:
        ...
        colDataLst = request.POST['colDataLst']
        uid=request.session['uid']
        api_url=getAPIURL()+"saveFLRatings/"       
        data_to_save={'colDataLst':colDataLst,             
            'uid':request.session['uid'], } 
        header = {
        "Content-Type":"application/json",
...
def FetchResidualRating(request):
    ...
    api_url=getAPIURL()+"FetchResidualRating/"       
    data_to_get={
...
def fetchfielddata(request):
    ...
    try:
        third_party_api_url_a = getAPIURL()+'FieldAPI/'
        data_to_get={ 
            'department': department,
            'portfolio':portfolio,
        } 
...
def valuedetails(request):
    try:
        ...
        return render(request, 'value_details.html',{'department':data})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())
    ...
    try: 
        ...
        for i in datalist: 
            ...
            data_to_save={ 
                'department': department,
                'field_aid':int(i['Field_AID']),
                'database_fields':i['Database_Field'],
                'operator':i['Operator'],
                'value':i['Value'],
                'portfolio':portfolio,
                'addedby':request.session['uid']
...
def showControllClass(request):
    try:     
        #new
        third_party_api_url = getAPIURL()+'ControllClassAPI/'
        header = {
        "Content-Type":"application/json",
	    'Authorization': 'Token '+request.session['accessToken']
        }
        response = requests.get(third_party_api_url, headers=header)
        print("response controll class",response.content)

        return render(request, 'controllclasslist.html',{ 'actPage':'Departments','data':json.loads(response.content)})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())

def newcontrollclass(request):
    try:   
        return render(request, 'addcontrollclass.html',{ 'actPage':'Add Department'})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())

def addcontrollclass(request):
    try:
        updated_id = request.POST.get('update_id','False')
...
def edit_controllclass(request,id): 
    try:
        # finddval_obj=Department.objects.get(dept_aid=id) 
        third_party_api_url = getAPIURL()+'ControllClassAPI/'+str(id)
        header = {
        "Content-Type":"application/json",
            'Authorization': 'Token '+request.session['accessToken']
        }
        response = requests.get(third_party_api_url, headers=header)
        print("response--------------------------qwer",json.loads(response.content))  
        controll_class_obj = json.loads(response.content)
        label=controll_class_obj['data']['label']
        desc=controll_class_obj['data']['description']
        return render(request, 'addcontrollclass.html',{ 'actPage':'Edit Controll Class Master','label':label,'description':desc,'id':id})
...
def showriskmaster(request):
    try:     
        #new
        third_party_api_url = getAPIURL()+'RiskMasterAPI/'
        header = {
        "Content-Type":"application/json",
	    'Authorization': 'Token '+request.session['accessToken']
        }
        response = requests.get(third_party_api_url, headers=header)
        print("response risk",response.content)

        return render(request, 'riskmasterlist.html',{ 'actPage':'Risk Master','data':json.loads(response.content)})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())

def newriskmaster(request):
    try:   
        return render(request, 'addriskmaster.html',{ 'actPage':'Add Risk master'})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())

def addriskmaster(request):
    try:
        third_party_api_url = getAPIURL()+'RiskMasterAPI/'
...
def edit_riskmaster(request,id): 
    try:
        # finddval_obj=Department.objects.get(dept_aid=id) 
        third_party_api_url = getAPIURL()+'RiskMasterAPI/'+str(id)
        header = {
        "Content-Type":"application/json",
            'Authorization': 'Token '+request.session['accessToken']
        }
        response = requests.get(third_party_api_url, headers=header)
        print("response--------------------------qwer",json.loads(response.content))  
        riskmaster_obj = json.loads(response.content)
        utility=riskmaster_obj['data']['utility']
        label=riskmaster_obj['data']['label']
        desc=riskmaster_obj['data']['description']
        return render(request,'addriskmaster.html',{ 'actPage':'Edit Risk Master','label':label,'description':desc,'id':id})
...
def Get_Utility(request):
    try: 
        ...
        print("request_data",utility)
        prefix = list(utility)[0]
        if utility == "Pricing":
            print("if--------")
            api_url=getAPIURL()+"GetUtilityAPI/"       
            data_to_get={ 
                'utility':utility
                } 
            header = {
            ...
            print("api_data_utility-------------",api_data)
...
def saveriskcomments(request):
    ...
    api_url=getAPIURL()+"saveriskcomments/"       
    data_to_save={ 
        'uid':request.session['uid'],
        'comments':comment,
        'risk_id':risk_id,
...
def getriskfactorhistorymsg(request):
    try:
        ...
        third_party_api_url = getAPIURL()+'GetRiskFactorHistoryMsg/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
...
def addgetriskfactorhistorymsg(request):
    try:
        ...
        responseget = requests.post(third_party_api_url, data= json.dumps(data_to_save),headers=header)
        data=responseget.json()
        print("responseget save data--------------------------",data)            
        return JsonResponse({'data':data['data']})
    except Exception as e:
        print("error is",e)


def getFLSub_Sub_Sections(request):
    try:       
        sub_secid =request.GET.get('secid', 'False')      
        
        return JsonResponse({'subsections':objmaster.getFLSub_Sub_Sections(sub_secid)})
    except Exception as e:
        print('addSection is ',e)
        print('addSection traceback is ', traceback.print_exc()) 
...
def showFLSetting(request):
    try: 
        third_party_api_url = getAPIURL()+'FLSettingAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
...
def AssignmentRiskFactor(request):
    try:
        # mdl_id=request.GET.get('mdl_id','')
        # print('mdl_id is ',mdl_id)
        # api_url=getAPIURL()+"reqValidation/"       
        ...
        print("api_data_utility only-------------",api_data)
        return render(request, 'AssignmentRiskFactor.html',{ 'actPage':'Risk Factor Assignement','utility':api_data['data'],'mrmUsers':api_data['mrmUsers'],'range_utility_max_cnt':range(1,api_data['utility_max_count']+1)})
    except Exception as e:
        print('AssignmentRiskMaster is ',e)
        print('AssignmentRiskMaster traceback is ', traceback.print_exc())


def FetchRiskFactorUtility(request):
    print()
    utility =request.GET.get('utility', 'False') 
    print("request_data",utility)
    prefix = list(utility)[0]
    try:
        ...
        print("api_data_utility-------------",api_data)
        return JsonResponse({'data':api_data['data'],'prefix':prefix,'title':'Risk Factor','utility':utility})
    except Exception as e:
        print("error is",e)

def Add_Risk_Factor_Assignment(request):
    print("request_data Risk FActor",request.POST)
    try: 
        datalist = json.loads(request.GET['datalist'])
        print("datalist",datalist)
        utility = request.GET['utility']
        print("utility",utility)
        api_url=getAPIURL()+"AssignRiskFactorAPI/"  
        for i in datalist: 
            print("i_data",i)
            parsed_date = datetime.strptime(i['end_date'], "%m/%d/%Y")
            converted_date = parsed_date.strftime("%Y-%m-%d")
            for assignto in i['assign_to']:
                data_to_save={ 
                    'risk_aid':i['risk_aid'],
                    'risk_factor_label':i['request_for_label'],
                    'assign_to':assignto,
                ...
                response = requests.post(api_url, data= json.dumps(data_to_save),headers=header)  
                print("response risk factor",json.loads(response.content))
                if json.loads(response.content)['msg'] == 'Risk Factor Assignment is saved Successufully':
                    istaken = "true"
                else:
                    istaken = "false"
        return JsonResponse({'istaken':istaken,'msg':json.loads(response.content)['msg']})
    except Exception as e:
        print('error is ',e)
        print('error traceback is ', traceback.print_exc()) 
        return JsonResponse({'istaken':'false','msg':'Something went wrong'})
...
def question_ans(request):
    try:
        ...
        if request.method == 'POST':
            ...
            api_url=getAPIURL()+"add_question_ans/"       
...
def generate_pdf_underwritting(pdf):
    # try:
    # class_var = ['derived_race','derived_sex','derived_ethnicity','applicant_age']
    # datalist = []
    # for i in class_var:
    #     api_url = getAPIURL()+'get_FL_Report_Data_Varwise/' 
    #     header = {
    #     "Content-Type":"application/json", 
    #     'Authorization': 'Token '+request.session['accessToken']
    #     }       
    ...
    datalist =[{'istaken': 'true', 'apps': {'Dropout': {'0': {'ordCol': 1, 'ClsLbl': 'Male:White', 'ttlappl': 2462, 'Originations': 1153.0, 'Denied': 0, 'Dropped': 0,'AIR':198,'DOR':789,'DOI':179}, '1': {'ordCol': 2, 'ClsLbl': '2 or more minority races', 'ttlappl': 1, 'Originations': None, 'Denied': 1, 'Dropped': 0,'AIR':222,'DOR':786,'DOI':123}, '2': {'ordCol': 2, 'ClsLbl': 'American Indian or Alaska Native', 'ttlappl': 12, 'Originations': 3.0, 'Denied': 5, 'Dropped': 4,'AIR':554,'DOR':723,'DOI':455}, '3': {'ordCol': 2, 'ClsLbl': 'Asian', 'ttlappl': 371, 'Originations': 137.0, 'Denied': 180, 'Dropped': 52,'AIR':227,'DOR':265,'DOI':289}, '4': {'ordCol': 2, 'ClsLbl': 'Black or African American', 'ttlappl': 383, 'Originations': 98.0, 'Denied': 249, 'Dropped': 33,'AIR':198,'DOR':789,'DOI':179}, '5': {'ordCol': 2, 'ClsLbl': 'Joint', 'ttlappl': 94, 'Originations': 44.0, 'Denied': 39, 'Dropped': 11,'AIR':198,'DOR':789,'DOI':179}, '6': {'ordCol': 2, 'ClsLbl': 'Native Hawaiian or Other Pacific Islander', 'ttlappl': 21, 'Originations': 5.0, 'Denied': 13, 'Dropped': 3,'AIR':198,'DOR':789,'DOI':179}, '7': {'ordCol': 2, 'ClsLbl': 'Race Not Available', 'ttlappl': 1108, 'Originations': 569.0, 'Denied': 422, 'Dropped': 105,'AIR':198,'DOR':789,'DOI':179}, '8': {'ordCol': 2, 'ClsLbl': 'White', 'ttlappl': 5476, 'Originations': 3192.0, 'Denied': 1826, 'Dropped': 426,'AIR':198,'DOR':789,'DOI':179}},'title_comment':{'title':'HDFC ','comment':'Analysis and porfolio of department is completed '}},'filterSelected': " Portfolio ='Mortgage' and department='MRMedited' "},

    {'istaken': 'true', 'apps': {'Dropout': {'0': {'ordCol': 1, 'ClsLbl': 'White:Male', 'ttlappl': 2462, 'Originations': 1153, 'Denied': 0, 'Dropped': 0,'AIR':198,'DOR':789,'DOI':179}, '1': {'ordCol': 2, 'ClsLbl': 'Female', 'ttlappl': 1748, 'Originations': 760, 'Denied': 827, 'Dropped': 150,'AIR':198,'DOR':789,'DOI':179}, '2': {'ordCol': 2, 'ClsLbl': 'Joint', 'ttlappl': 4409, 'Originations': 2674, 'Denied': 1345, 'Dropped': 366,'AIR':198,'DOR':789,'DOI':179}, '3': {'ordCol': 2, 'ClsLbl': 'Male', 'ttlappl': 357, 'Originations': 110, 'Denied': 207, 'Dropped': 38,'AIR':198,'DOR':789,'DOI':179}, '4': {'ordCol': 2, 'ClsLbl': 'Sex Not Available', 'ttlappl': 952, 'Originations': 504, 'Denied': 356, 'Dropped': 80,'AIR':198,'DOR':789,'DOI':179}},'title_comment':{'title':'HDFC','comment':'Analysis and porfolio of department is completed'}}, 'filterSelected': " Portfolio ='Mortgage' and department='MRMedited' "}, 

    {'istaken': 'true', 'apps': {'Dropout': {'0': {'ordCol': 1, 'ClsLbl': 'White:Male', 'ttlappl': 2462, 'Originations': 1153, 'Denied': 0, 'Dropped': 0,'AIR':198,'DOR':789,'DOI':179}, '1': {'ordCol': 2, 'ClsLbl': 'Ethnicity Not Available', 'ttlappl': 1078, 'Originations': 562, 'Denied': 404, 'Dropped': 100,'AIR':198,'DOR':789,'DOI':179}, '2': {'ordCol': 2, 'ClsLbl': 'Hispanic or Latino', 'ttlappl': 143, 'Originations': 37, 'Denied': 87, 'Dropped': 19,'AIR':198,'DOR':789,'DOI':179}, '3': {'ordCol': 2, 'ClsLbl': 'Joint', 'ttlappl': 54, 'Originations': 28, 'Denied': 19, 'Dropped': 7,'AIR':198,'DOR':789,'DOI':179}, '4': {'ordCol': 2, 'ClsLbl': 'Not Hispanic or Latino', 'ttlappl': 6191, 'Originations': 3421, 'Denied': 2225, 'Dropped': 508,'AIR':198,'DOR':789,'DOI':179}},'title_comment':{'title':'HDFC','comment':'Analysis and porfolio of department is completed'}}, 'filterSelected': " Portfolio ='Mortgage' and department='MRMedited' "},

    {'istaken': 'true', 'apps': {'Dropout': {'0': {'ordCol': 1, 'ClsLbl': 'White:Male', 'ttlappl': 2462, 'Originations': 1153, 'Denied': 0, 'Dropped': 0,'AIR':198,'DOR':789,'DOI':179}, '1': {'ordCol': 2, 'ClsLbl': '<25', 'ttlappl': 27, 'Originations': 4, 'Denied': 19, 'Dropped': 4,'AIR':198,'DOR':789,'DOI':179}, '2': {'ordCol': 2, 'ClsLbl': '>74', 'ttlappl': 526, 'Originations': 284, 'Denied': 181, 'Dropped': 57,'AIR':198,'DOR':789,'DOI':179}, '3': {'ordCol': 2, 'ClsLbl': '25-34', 'ttlappl': 581, 'Originations': 269, 'Denied': 257, 'Dropped': 55,'AIR':198,'DOR':789,'DOI':179}, '4': {'ordCol': 2, 'ClsLbl': '35-44', 'ttlappl': 1388, 'Originations': 768, 'Denied': 503, 'Dropped': 107,'AIR':198,'DOR':789,'DOI':179}, '5': {'ordCol': 2, 'ClsLbl': '45-54', 'ttlappl': 1794, 'Originations': 966, 'Denied': 694, 'Dropped': 128,'AIR':198,'DOR':789,'DOI':179}, '6': {'ordCol': 2, 'ClsLbl': '55-64', 'ttlappl': 1701, 'Originations': 877, 'Denied': 649, 'Dropped': 163,'AIR':198,'DOR':789,'DOI':179}, '7': {'ordCol': 2, 'ClsLbl': '65-74', 'ttlappl': 1295, 'Originations': 748, 'Denied': 419, 'Dropped': 117,'AIR':198,'DOR':789,'DOI':179}, '8': {'ordCol': 2, 'ClsLbl': '8888', 'ttlappl': 154, 'Originations': 132, 'Denied': 13, 'Dropped': 3,'AIR':198,'DOR':789,'DOI':179}},'title_comment':{'title':'HDFC','comment':'Analysis and porfolio of department is completed'}}, 'filterSelected': " Portfolio ='Mortgage' and department='MRMedited' "}]
    # rpt_data= {'Dropout': {'0': {'ordCol': 1, 'ClsLbl': 'White:Male', 'ttlappl': 2462, 'Originations': 1153, 'Denied': 0, 'Dropped': 0,'AIR':198,'DOR':789,'DOI':179}, '1': {'ordCol': 2, 'ClsLbl': 'Ethnicity Not Available', 'ttlappl': 1078, 'Originations': 562, 'Denied': 404, 'Dropped': 100,'AIR':154,'DOR':369,'DOI':720}, '2': {'ordCol': 2, 'ClsLbl': 'Hispanic or Latino', 'ttlappl': 143, 'Originations': 37, 'Denied': 87, 'Dropped': 19,'AIR':200,'DOR':456,'DOI':379}, '3': {'ordCol': 2, 'ClsLbl': 'Joint', 'ttlappl': 54, 'Originations': 28, 'Denied': 19, 'Dropped': 7,'AIR':526,'DOR':596,'DOI':781}, '4': {'ordCol': 2, 'ClsLbl': 'Not Hispanic or Latino', 'ttlappl': 6191, 'Originations': 3421, 'Denied': 2225, 'Dropped': 508,'AIR':222,'DOR':556,'DOI':109}},'title_comment':{'title':'check new','comment':'check new comment'}}
    #rpt_data['Dropout']['0']['ClsLbl']
    # x,y = 25,50
    # # pdf = FPDF(orientation = 'P', unit = 'mm', format='Legal')#PDF()
    # bank_name = "HDFC Bank"
    # title = "Overall Application Summary for "+bank_name
    ...
    for i,rpt_data in enumerate(datalist):
        ...
        ### Minority Sum ###
        application_values = [j['ttlappl'] for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        print("ttlapp value",application_values)
        ...
        for  i,j in rpt_data['apps']['Dropout'].items():
            if i != '0':
                print("j['Originations']",j['Originations'])
                if j['Originations'] == None:
        ...
        print("OriginationRate_values",OriginationRate_values)

        Denials_values = [j['Denied'] for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        DenialsRate_values = [round(j['Denied']/j['ttlappl'],2)*100 for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        Droupout_values = [j['Dropped'] for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        DroupoutRate_values = [round(j['Dropped']/j['ttlappl'],2)*100 for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        air_values =  [j['AIR'] for i,j in  rpt_data['apps']['Dropout'].items() if i != '0']
        dor_values =  [j['DOR'] for i,j in  rpt_data['apps']['Dropout'].items() if i != '0']
        doi_values =  [j['DOI'] for i,j in  rpt_data['apps']['Dropout'].items() if i != '0']
        ...
        pdf.cell(25, 10, str(sum(Origination_values)),
                1, fill=True, align='C')
        pdf.set_xy(95, y)   ##
        pdf.multi_cell(25, 10, str(round(sum(Origination_values)/sum(application_values),2)*100),
                1, fill=True, align='C')
        pdf.set_xy(120, y)
        pdf.cell(25, 10,str(sum(Denials_values)),
                1, fill=True, align='C')
        pdf.set_xy(145, y)
        pdf.multi_cell(25, 10,str(round(sum(Denials_values)/sum(application_values),2)*100),
                1, fill=True, align='C')
        pdf.set_xy(170, y)
        pdf.cell(20, 10, str(sum(Droupout_values)), 1, fill=True, align='C')
        pdf.set_xy(190, y)
        pdf.multi_cell(20, 10, str(round(sum(Droupout_values)/sum(application_values),2)*100), 1, fill=True, align='C')
        ...
        pdf.multi_cell(25, 10, str(ctrlCls['Originations']),
                1,fill=True, align='C')
        pdf.set_xy(95, y) 
        pdf.multi_cell(25, 10,  str(round(ctrlCls['Originations']/ctrlCls['ttlappl'],2)*100),
                1,fill=True, align='C')
        pdf.set_xy(120, y)
        ...
        pdf.multi_cell(20, 10, str(round(ctrlCls['Dropped']/ctrlCls['ttlappl'],2)*100), 1,fill=True, align='C')
        ...
        pdf.set_xy(70, y)
        if round(ctrlCls['Denied']/ctrlCls['ttlappl'],2)*100 == 0:
        ...
        pdf.set_xy(95, y)
        if round(ctrlCls['Dropped']/ctrlCls['ttlappl'],2*100) == 0:
            pdf.cell(25, 10, "NA", 1, fill=True, align='C')
        else:
        ...
        # y = pdf.get_y()+10
        # pdf.set_xy(10, y)
        # pdf.multi_cell(35, 10, str(ctrlCls['ClsLbl']),1,
        #         fill=True, align='C')
        # pdf.set_font("Arial",size=9)      
        # pdf.set_xy(45, y)
        # pdf.cell(25, 10, str(ctrlCls['AIR']), 1,
        #         fill=True, align='C')
        # pdf.set_xy(70, y)
        # pdf.cell(25, 10, str(ctrlCls['DOR']), 1,
        #         fill=True, align='C')
        # pdf.set_xy(95, y)
        # pdf.cell(25, 10, str(ctrlCls['DOI']), 1,
        #         fill=True, align='C')
        #Add Footer
        pdf.set_xy(x, y)
        y = pdf.get_y()    
        ...
        for i,j in rpt_data['apps']['Dropout'].items():
            ...
            if i != '0':    
                ...
                y = pdf.get_y()#+10
                pdf.set_xy(10, y)
                print("string length",str(j['ClsLbl']),len(str(j['ClsLbl'])))
                if len(str(j['ClsLbl'])) > 18:
                ...
                if j['ttlappl'] == 0:
                    ...
                    if j['Originations'] == None:                        
                        pdf.multi_cell(25, 10, str(round(0/j['ttlappl'],2)*100),
                            1, fill=True, align='C')
                    else:
                        pdf.multi_cell(25, 10, str(round(j['Originations']/j['ttlappl'],2)*100),
                            1, fill=True, align='C')
                pdf.set_xy(120, y)
                ...
                if j['Originations'] == None:
                    ...
                    print("Originations",j['Originations'],"ttlappl",j['ttlappl'])
                    print("Originations_wm",ctrlCls['Originations'],"ttlappl_wm",ctrlCls['ttlappl'])   
                    origin_val =   j['Originations']/j['ttlappl']*100  
                    origin_val_vm =  ctrlCls['Originations']/ctrlCls['ttlappl']*100  
                    print("-----og_val",round(origin_val),"og_val_2",round(origin_val_vm),"val_solution",round(origin_val/origin_val_vm*100)) 
    ...
    return pdf    
    # pdf.output(os.path.join(
    #         BASE_DIR, "static/medtable_from_dict.pdf"))
...
def generate_pdf_Steering(pdf):

    datalist =[{'istaken': 'true', 'apps': {'Dropout': 
    {'0': 
    {'ordCol': 1, 'ClsLbl': 'Male:White', 'Applications': 2462, 'Applications_Converted': 1153.0, 'Peer_Applications_Converted': 2223, 'Application_Conversion_Rate': 7899,'Peer_Application_Conversion_Rate':198,'Government_Applications':789,'Government_Application_perct':179,'Peer_Government_Applications':4442,'Peer_Government_Application_perct':4455},

    '1': {'ordCol': 2, 'ClsLbl': '2 or more minority races', 'Applications': 8885, 'Applications_Converted': 1153.0, 'Peer_Applications_Converted': 2223, 'Application_Conversion_Rate': 7899,'Peer_Application_Conversion_Rate':198,'Government_Applications':789,'Government_Application_perct':179,'Peer_Government_Applications':4442,'Peer_Government_Application_perct':4455},

    '2': {'ordCol': 2, 'ClsLbl': 'American Indian or Alaska Native', 'Applications': 1236, 'Applications_Converted': 1153.0, 'Peer_Applications_Converted': 2223, 'Application_Conversion_Rate': 7899,'Peer_Application_Conversion_Rate':198,'Government_Applications':789,'Government_Application_perct':179,'Peer_Government_Applications':4442,'Peer_Government_Application_perct':4455}, 

    '3': {'ordCol': 2, 'ClsLbl': 'Asian', 'Applications': 2223, 'Applications_Converted': 1153.0, 'Peer_Applications_Converted': 2223, 'Application_Conversion_Rate': 7899,'Peer_Application_Conversion_Rate':198,'Government_Applications':789,'Government_Application_perct':179,'Peer_Government_Applications':4442,'Peer_Government_Application_perct':4455}},
    'title_comment':{'title':'HDFC ','comment':'Analysis and porfolio of department is completed '}},'filterSelected': " Portfolio ='Mortgage' and department='MRMedited' "},

    {'istaken': 'true', 'apps': {'Dropout': {
    '0': {'ordCol': 1, 'ClsLbl': 'White:Male','Applications': 2223, 'Applications_Converted': 1153.0, 'Peer_Applications_Converted': 2223, 'Application_Conversion_Rate': 7899,'Peer_Application_Conversion_Rate':198,'Government_Applications':789,'Government_Application_perct':179,'Peer_Government_Applications':4442,'Peer_Government_Application_perct':4455}, 
    '1': {'ordCol': 2, 'ClsLbl': 'Female','Applications': 2223, 'Applications_Converted': 1153.0, 'Peer_Applications_Converted': 2223, 'Application_Conversion_Rate': 7899,'Peer_Application_Conversion_Rate':198,'Government_Applications':789,'Government_Application_perct':179,'Peer_Government_Applications':4442,'Peer_Government_Application_perct':4455}, 
    '2': {'ordCol': 2, 'ClsLbl': 'Joint','Applications': 2223, 'Applications_Converted': 1153.0, 'Peer_Applications_Converted': 2223, 'Application_Conversion_Rate': 7899,'Peer_Application_Conversion_Rate':198,'Government_Applications':789,'Government_Application_perct':179,'Peer_Government_Applications':4442,'Peer_Government_Application_perct':4455},
    '3': {'ordCol': 2, 'ClsLbl': 'Male', 'Applications': 2223, 'Applications_Converted': 1153.0, 'Peer_Applications_Converted': 2223, 'Application_Conversion_Rate': 7899,'Peer_Application_Conversion_Rate':198,'Government_Applications':789,'Government_Application_perct':179,'Peer_Government_Applications':4442,'Peer_Government_Application_perct':4455}},'title_comment':{'title':'HDFC','comment':'Analysis and porfolio of department is completed'}}, 'filterSelected': " Portfolio ='Mortgage' and department='MRMedited' "}]
    

    for i,rpt_data in enumerate(datalist):
        ### Minority Sum ###
        application_values = [j['Applications'] for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        print("ttlapp value",application_values)
        Applications_Converted = []
        for  i,j in rpt_data['apps']['Dropout'].items():
            if i != '0':
                if j['Applications_Converted'] == None:
                    Applications_Converted_2 = 0
                    a = Applications_Converted_2
                else:
                    a = j['Applications_Converted']
                Applications_Converted.append(a)
        print("Applications_Converted",Applications_Converted)
        Peer_Applications_Converted = [j['Peer_Applications_Converted'] for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        Application_Conversion_Rate = [j['Application_Conversion_Rate'] for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        Peer_Application_Conversion_Rate = [j['Peer_Application_Conversion_Rate'] for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        Government_Applications = [j['Government_Applications'] for i,j in rpt_data['apps']['Dropout'].items() if i != '0']
        Government_Application_perct =  [j['Government_Application_perct'] for i,j in  rpt_data['apps']['Dropout'].items() if i != '0']
        Peer_Government_Applications =  [j['Peer_Government_Applications'] for i,j in  rpt_data['apps']['Dropout'].items() if i != '0']
        Peer_Government_Application_perct =  [j['Peer_Government_Application_perct'] for i,j in  rpt_data['apps']['Dropout'].items() if i != '0']
        ...
        pdf.multi_cell(25,10, "Application",
                1, fill=True, align='C')
        pdf.set_xy(70, y)
        pdf.multi_cell(35, 5,  "Applications Converted",
                1, fill=True, align='C')
        pdf.set_xy(105, y)
        pdf.multi_cell(35, 5,  "Peer Applications Converted",
                1, fill=True, align='C')
        pdf.set_xy(140, y)
        pdf.multi_cell(35, 5,"Application Conversion Rate",
                1, fill=True, align='C')
        pdf.set_xy(175, y)
        pdf.multi_cell(35, 5,"Peer Application Conversion Rate",
                1, fill=True, align='C')
        ...
        pdf.set_xy(70, y)
        print("Applications_Converted",Applications_Converted)
        pdf.cell(35, 10, str(sum(Applications_Converted)),
                1, fill=True, align='C')
        pdf.set_xy(105, y) ##
        pdf.multi_cell(35, 10, str(sum(Peer_Applications_Converted)),
                1, fill=True, align='C')
        pdf.set_xy(140, y)
        pdf.cell(35, 10,str(sum(Application_Conversion_Rate)),
                1, fill=True, align='C')
        pdf.set_xy(175, y)
        pdf.multi_cell(35, 10,str(sum(Peer_Application_Conversion_Rate)),
                1, fill=True, align='C')
        ...
        pdf.multi_cell(25, 10, str(ctrlCls['Applications']),
                1,fill=True, align='C')
        pdf.set_xy(70, y)
        pdf.multi_cell(35, 10, str(ctrlCls['Applications_Converted']),
                1,fill=True, align='C')
        pdf.set_xy(105, y) 
        pdf.multi_cell(35, 10,  str(ctrlCls['Peer_Applications_Converted']),
                1,fill=True, align='C')
        pdf.set_xy(140, y)
        pdf.multi_cell(35, 10,str(ctrlCls['Application_Conversion_Rate']),
                1,fill=True, align='C')
        pdf.set_xy(175, y) 
        pdf.multi_cell(35, 10, str(ctrlCls['Peer_Application_Conversion_Rate']),
                1,fill=True, align='C')
        ...
        pdf.multi_cell(35,10, "",
                1,fill=True, align='C')
        pdf.set_xy(45, y)
        pdf.multi_cell(35, 5, "Government Applications",1, fill=True, align='C')
        pdf.set_xy(80, y)
        pdf.multi_cell(35, 5,"Government Application perct" , 1,
                fill=True, align='C')
        pdf.set_xy(115, y)
        pdf.multi_cell(35, 5,"Peer Government Applications" , 1,
                fill=True, align='C')
        pdf.set_xy(150, y)
        pdf.multi_cell(35, 5, "Peer Government Application perct", 1,
                fill=True, align='C') 
        ...
        pdf.set_font("Arial",size=9) 
        pdf.set_xy(45, y)  
        pdf.cell(35, 10, str(sum(Government_Applications)), 1, fill=True, align='C')
        pdf.set_xy(80, y)
        pdf.multi_cell(35, 10, str(sum(Government_Application_perct)),1,
                fill=True, align='C')     
        pdf.set_xy(115, y)
        pdf.cell(35, 10, str(sum(Peer_Government_Applications)), 1,
                fill=True, align='C')
        pdf.set_xy(150, y)
        pdf.cell(35, 10, str(sum(Peer_Government_Applications)), 1,
                fill=True, align='C')
        ...
        pdf.set_font("Arial",size=9)
        pdf.set_xy(45, y)  
        pdf.cell(35, 10, str(ctrlCls['Government_Applications']), 1, fill=True, align='C')
        pdf.set_xy(80, y)
        pdf.multi_cell(35, 10, str(ctrlCls['Government_Application_perct']),1,
                fill=True, align='C')      
        pdf.set_xy(115, y)
        pdf.cell(35, 10, str(ctrlCls['Peer_Government_Applications']), 1,
                fill=True, align='C')
        pdf.set_xy(150, y)
        pdf.cell(35, 10, str(ctrlCls['Peer_Government_Application_perct']), 1,
                fill=True, align='C')
        ...
        for i,j in rpt_data['apps']['Dropout'].items():
            ...
            if i != '0':    
                ...
                pdf.multi_cell(25,10, "Application",
                        1, fill=True, align='C')
                pdf.set_xy(70, y)
                pdf.multi_cell(35, 10,  "Applications Converted",
                        1, fill=True, align='C')
                pdf.set_xy(105, y)
                pdf.multi_cell(35, 5,  "Peer Applications Converted",
                        1, fill=True, align='C')
                ...
                pdf.cell(25, 10, str(j['Applications']),
                        1, fill=True, align='C')
                pdf.set_xy(70, y)
                pdf.cell(35, 10, str(j['Applications_Converted']),
                        1, fill=True, align='C')
                pdf.set_xy(105, y)   ##
                pdf.multi_cell(35, 10, str(j['Peer_Applications_Converted']),
                            1, fill=True, align='C')
                pdf.set_xy(140, y)
                pdf.cell(35, 10,str(j['Application_Conversion_Rate']),
                        1, fill=True, align='C')
                pdf.set_xy(175, y)
                pdf.multi_cell(35, 10,str(j['Peer_Application_Conversion_Rate']),
                            1, fill=True, align='C')
                ...
                pdf.multi_cell(35, 5, "Government Applications",1, fill=True, align='C')
                pdf.set_xy(80, y)
                pdf.multi_cell(35, 5, "Government Application perct", 1, fill=True, align='C')
                ...
                pdf.set_font("Arial",size=9)
                pdf.set_xy(45, y)
                pdf.cell(35, 10, str(j['Government_Applications']), 1, fill=True, align='C')
                pdf.set_xy(80, y)
                pdf.multi_cell(35, 10, str(j['Government_Application_perct']), 1, fill=True, align='C')
                pdf.set_xy(115, y)
                pdf.cell(35, 10, str(ctrlCls['Peer_Government_Applications']), 1,
                        fill=True, align='C')
                pdf.set_xy(150, y)
...
def Fl_titleCommentSave(request):
    ...
    api_url=getAPIURL()+"FlReportTitleCommentAPI/"       
    data_to_save={ 
        'comment':comments,
        'title':title,
        'utility':utility,
...
def insert_data_to_sql(request):
    ...
    try:
        ...
        modelfile=file_path+ "output_chk.xlsx"  
        # myfile = request.FILES['myfile']
        # fs = FileSystemStorage()
        # file_path = fs.save(myfile.name, myfile)
        # uploaded_file_path = fs.path(file_path)
        new_df = pd.read_csv(modelfile)
        print("new dataframe",new_df)
        # csv_file_path = os.path.join(settings.BASE_DIR, 'modelval/static/data.csv')
# excel_fields = new_df.columns.tolist()
        # print("excel_fields",excel_fields)

        # third_party_api_url  = getAPIURL()+'MappingAPI/'
        ...
        #     'department':"MRM",
        #     'portfolio':"MRM",
        #     'filepath':"C:/Jayesh/OwnProjects/forntendrmse/modelval/static/data.csv"
        #     } 
        # header = {
        # "Content-Type":"application/json",
	    # 'Authorization': 'Token '+request.session['accessToken']
...
def file_raw_fl(request):
    ...
    if request.method == 'POST' and request.FILES['file']:
        ...
        if upload_type == 'raw':
            ...
            return HttpResponse("Invalid upload type", status=400)
        ...
        api_url=getAPIURL()+"fl_data_file_info/"       
        ...
        file_name = folder +'/'+'output_chk.xlsx'
        df_new_1 = pd.read_excel(file_name)
        df_new =  df_new_1.to_dict(orient="records")
        print("dataframe check",df_new)

        api_url=getAPIURL()+"FlImportData/"       
        params={
            'df':df_new
            } 
        header = {
        ...
        response_1 = requests.post(api_url, data= json.dumps(params),headers=header)         
        api_data_1=response_1.json()
        print("api_data_1",api_data_1)
...
def residualratingslist(request):
    try:  
        print() 
        third_party_api_url = getAPIURL()+'InherentRiskRatingAPI/'
        header = {
        "Content-Type":"application/json",
        'Authorization': 'Token '+request.session['accessToken']
        }
        ...
        return render(request, 'residualratingslist.html',{'ratings_1':json.loads(r1),'ratings_2':json.loads(r2)})
    except Exception as e:
        print('setuppycaret is ',e)
        print('setuppycaret traceback is ', traceback.print_exc()) 
...
def saveresidualrating(request):
    ...
    api_url=getAPIURL()+"SaveResidualRatingsAPI/"       
    data_to_save={ 
...
def showinherentristrating(request):
    try: 
        #new
        third_party_api_url = getAPIURL()+'ALLInherentRatingAPI/'
        header = {
        "Content-Type":"application/json",
	    'Authorization': 'Token '+request.session['accessToken']
        } 
...
def edit_inherentristrating(request,id): 
    try:
        print("check id",id)
        # finddval_obj=Department.objects.get(dept_aid=id) 
        third_party_api_url = getAPIURL()+'ALLInherentRatingAPI/'+str(id)
        header = {
        "Content-Type":"application/json",
            'Authorization': 'Token '+request.session['accessToken']
        }
...
def showcontroleffectiverating(request):
    try: 
        #new
        third_party_api_url = getAPIURL()+'ControlEffectiveRatingAPI/'
        header = {
        "Content-Type":"application/json",
	    'Authorization': 'Token '+request.session['accessToken']
        } 
...
def edit_controleffectiverating(request,id): 
    try:
        print("check id",id)
        # finddval_obj=Department.objects.get(dept_aid=id) 
        third_party_api_url = getAPIURL()+'ControlEffectiveRatingAPI/'+str(id)
        header = {
        "Content-Type":"application/json",
            'Authorization': 'Token '+request.session['accessToken']
        }
