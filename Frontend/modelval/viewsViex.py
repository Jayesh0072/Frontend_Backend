...
from .modelview import Send_process_status
from .views import find_max_file_id,find_target_value,find_src_data
...
def table(request):
    try: 
        ...
        if request.method == 'POST' and request.FILES['myfile']:
            ...
            if os.path.exists(savefile_name):
                ...
                for i1, i2, chunk in gen:
                    ...
                    abk=list()
                    file_obj=collection_file_info.find()
                    print("file obj",file_obj)
                    for k in file_obj:
                        abk.append(k)
                    print("abk",abk) 
                    print("abk length",len(abk))   

                    if len(abk) == 0:    
                        print("File id is default 1")
                        keys_data=list()
                        for i in data_model:
                            print("data inserting in SrcFileInfo")
                            #print("SrcFileInfo data",i)
                        keys_data.append(list(i.keys()))    ##column name
                        #print("keys_data",keys_data)
                        uploaded_on=datetime.datetime.now()  ##uploaded on data
                        file_name=myfile

                        file_info_data={'file_id':1,'file_columns':keys_data,'file_name':file_name,'uploaded_by':"User","uploaded_on":uploaded_on}
                        collection_file_info.insert_one(file_info_data)  
                        src_data_dict={}
                        for i in data_model:
                            print("data inserting in SrcData")
                            print("Src data",i)
                            src_data_dict=i
                            src_data_dict.update({'file_id':1})
                            print("src_data_dict",src_data_dict)
                        ...
                        print("more than 1",abk)
                        print("abk 0",abk[0])  
                        for i in abk:
                            old_file_id=i['file_id']
                        ...
                        file_info_data={'file_id':new_file_id,'file_name':file_name,'file_columns':keys_data,'uploaded_by':"User","uploaded_on":uploaded_on}
                        collection_file_info.insert_one(file_info_data)  
                        src_data_dict={}
                        for i in data_model:
                            print("data inserting in SrcData")
                            print("Src data",i)
                            src_data_dict=i
                            src_data_dict.update({'file_id':new_file_id})
                            print("src_data_dict",src_data_dict)
                            collection.insert_one(i)
                    dttypes = dict(df.dtypes)
                    # print('printing datatypes ') 
                ...
                #     file_info_data={'file_id':1,'file_columns':keys_data,'uploaded_by':"User","uploaded_on":uploaded_on}
                #     collection_file_info.insert_one(file_info_data)  
                # else:
                #     print("more than 1",abk)    
                del gen

            ## process status of import code 
            process_status=Send_process_status(Idx = 2,is_skipped="No")
        
        
        # result = collection_file_info.find({})
        # print('result is ,',result)
        # df =  pd.DataFrame(list(result))
        # print("dataframe",df["file_id"].max())
        # src_data_obj=collection.find({'file_id':int(df["file_id"].max())})
        # df= pd.DataFrame(list(src_data_obj))
        # print('dfdisplay is  ',dfdisplay)
        ...
        result = json.loads(result)    
        print("result type",type(result))
        dttypes = dict(dfdisplay.dtypes)
        print("dttypes",dttypes)
        for key, value in dttypes.items():
            gridDttypes.append({'colName': key, 'dataType': value})
       
        del df ,dfdisplay
        return render(request, 'showdata.html', {'isDisabled':_isDisabled,'desc': arrdescData, 'dataTypes': gridDttypes, 'df': result})#dfdisplay.to_html(classes="table table-striped")
...
def showCatCountPlot(request):
    try:
        ...
        # file1.close()
        file_id=find_max_file_id()
        targetVar=find_target_value()
        #print("targetVar",targetVar)
        x_keep=find_src_data(file_id)
        print("x_keep",x_keep)
        if not (targetVar=='None'):
            x_keep = x_keep.drop(targetVar, axis=1)
        # Retrieve all text variables from data
        cat_cols=get_cat_cols(x_keep)
...
def scattred3d(request):
    try:
        ...
        if(len(cat_cols)<1):
            return render(request, 'noCatVars.html')
        if(len(num_cols)<2):
            return render(request, 'noNumVars.html')
