...
def showSNSChart_vaex(request):
    try:
        ...
        print('time taken by vaex pairplot is ',time.time()-t1)
...
def showPieChart_vaex(request):
    try:
        ...
        context = {'graphpath': '\\static\\media\\'+user_name+'outputPieChart.png',
                   'plot_div': Markup(plot_div), 'hideddls': 'none', 'hideUnvar': 'flex', 'pageHeader': 'Pie Chart'}
        print('time taken by vaex pie chart is ',time.time()-t1)
...
def showDistPlot_vaex(request):
    ...
    print('time taken by vaex dist chart is ',time.time()-t1)
    del x_keep
...
def showBoxPlot_vaex(request):
    try:
        ...
        for i in num_cols:
            plt.subplot(math.ceil(len(num_cols)/4), 4, k)
            ax = sns.boxplot(x_keep[i].value_counts().values)
            ax.set_xlabel(i)
            k = k+1
        # plt.title('x_keep[i]', fontsize=10)
        fig.savefig(os.path.join(BASE_DIR, 'static\\media\\' +
                    user_name+'outputBoxPlot.png'))
        print('time taken by vaex box is ',time.time()-t1)
    ...
    try:
        ...
        for x in cat_cols_temp:
            if len(x_keep[x].value_counts())<25:
                cat_cols.append(x)
        print('time taken to get cols ',time.time()-t1)
        if(len(cat_cols) < 1):
        ...
        print('time taken to get count plot categorical ',time.time()-t1)
...
def plotinsoccuvsincstate_vaex(request):
    try:
        ...
        for x in cat_cols_temp:
            if len(x_keep[x].value_counts())<25:
                print('column is ',x,", ",x_keep[x].value_counts())
                cat_cols.append(x)
        if(len(cat_cols) < 1):
            return render(request, 'noCatVars.html')
        if(var1 == False):
