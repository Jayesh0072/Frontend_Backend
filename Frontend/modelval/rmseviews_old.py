...
#         else: 
#             print("add",uname,utype,lname,fname,email,StartDate,EndDate,dept_aid,activests)
...
#             response = requests.post(api_url,json=my_data, 
#                         headers={'content-type': 'application/json'})
#             print("responseeeeee",response.content)
#             print("status codeeeee",response.status_code)
...
def dashboard(request):
    try:  
        ...
        return render(request, 'dashboard_icon.html',{'icqratings':icqratings,'icq_exp':icq_exp,'modelinfo':modelinfo,'srcCnt':srcCnt,
                                                 ...
                                                 'toolCnt':toolCnt ,'findingsByElements':findingsByElements,
                                                 'modelttl':str(len(modelinfo)),'mdlRiskCnt':cnt,'findingsCntByCategory':findingsCntByCategory,
                                                 'modeltypes':modeltypes, 'actPage':'Quick View','activity_lst':json.dumps(latest)})
    except Exception as e:
        print('setuppycaret is ',e)
        print('setuppycaret traceback is ', traceback.print_exc()) 
        error_saving(request,e)
...
def updateMdlVersion(request):
    try:
        ...
        return JsonResponse({'newMdlId':objreg.updateMdlVersion(mdlid,isMinor),'regmdldata':api_data})
    except Exception as e:
        print('getMdlInfoById is ',e)
        print('getMdlInfoById traceback is ', traceback.print_exc())
...
def criteriaSetting(request):
    try: 
        print("criteriaSetting")
        qtnSections=objmaster.getQtnSections()
        tempCriteria=objmaster.getCriteria_sett(str(request.session['uid']))
        return render(request, 'criteriaSetting.html',{ 'actPage':'Set Criteria','tempCriteria':tempCriteria,'qtnSections':qtnSections})
    except Exception as e:
        print('criteriaQtns is ',e)
        print('criteriaQtns traceback is ', traceback.print_exc())

def criteriaSetting_temp(request):
    try: 
        print("criteriaSetting_temp")
        qtnSections=objmaster.getQtnSections()
...
def mmlabeltoexcel(request):
    try:
        ...
        return JsonResponse({'file': '/static/document_files/'+ mdlid +'/templates/' +mdlid+'.xlsx','msg':"Excel Downloaded successfully",'model_matrics':lst})
    except Exception as e:
        print('error is ',e)
...
#         return render(request, 'show_modelmetrics.html',{'actPage' :'PROPE- Frequency Function','mm_data':json.loads(response.content)})
#     except Exception as e:
#         print('adduser is ',e) 
#         print('adduser traceback is ', traceback.print_exc())
...
def edit_sub_task_type(request,id):
    ...
    try: 
        ...
        response = requests.get(third_party_api_url, headers=header)
        print("response edit_sub_task_type--------------------------",response.content)
...
def save_issue_function(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            active_status = request.POST.get('rbactivests')
            print('update_id',update_id)
...
def show_issue_priority(request):
    try: 
        ...
        print("show_issue_priority",'Token '+request.session['accessToken'])
        header = {
        "Content-Type":"application/json",
	    'Authorization': 'Token '+request.session['accessToken']
        }
...
def show_fl_section(request):
    try: 
        third_party_api_url = getAPIURL()+'section_master/'
        header = {
        "Content-Type":"application/json",
	    'Authorization': 'Token '+request.session['accessToken']
        }
        ...
        return render(request, 'show_fl_section.html',{'actPage' :'RMSE - Section','users':json.loads(response.content)})
    except Exception as e:
        print('adduser is ',e) 
        print('adduser traceback is ', traceback.print_exc())
...
def add_section(request):
    try:   
        return render(request, 'add_section.html',{'actPage':'RMSE - Add Issue Type'})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())
...
def save_section(request):
    try:
        print("---------save_section",request.POST)
...
def edit_section(request,id):
    ...
    third_party_api_url = getAPIURL()+'section_master/'+id
    ...
    data=json.loads(response.content)
    print("data",data)
    print("label",data['data']['section_label'])
    return render(request, 'add_section.html',{'section_label':data['data']['section_label'],
                                                     'section_description':data['data']['section_description'],'id':id})

def show_fl_sub_section(request):
    try: 
        third_party_api_url = getAPIURL()+'sub_section_master/'
        header = {
        "Content-Type":"application/json",
	    'Authorization': 'Token '+request.session['accessToken']
        }
        ...
        return render(request, 'show_fl_sub_section.html',{'actPage' :'RMSE - Sub Section','users':json.loads(response.content)})
    except Exception as e:
        print('adduser is ',e) 
        print('adduser traceback is ', traceback.print_exc())
...
def add_sub_section(request):
    try: 
        typeobj = FlSectionTraining.objects.all()
        return render(request, 'add_sub_section.html',{'actPage':'RMSE - Add Sub Issue Type','sections':typeobj,'type':'add'})
    except Exception as e:
        print('adduser is ',e)
        print('adduser traceback is ', traceback.print_exc())

def save_sub_section(request):
    print("save_sub_section")
    try:
        print("---------save_sub_section",request.POST)
    
        id=request.POST.get('update_id','False')
        sub_section_label = request.POST.get('label','False') 
        sub_section_description = request.POST.get('desc','False')
        section_aid = request.POST.get('issuetype','False')
        print("update_id",id)
        print("section_aid",section_aid)

        print("sub_section_label,sub_issue_type_description,issue_type",sub_section_label,sub_section_description,section_aid)
        if id != 'undefined':
            ...
            data_to_update = {
                'sub_section_label':sub_section_label,
                'sub_section_description':sub_section_description,
                'section_aid':section_aid,
                'sub_section_aid':id
            }
            header = {
            "Content-Type":"application/json",
...
def edit_sub_section(request,id):
    print("edit_sub_section",id)
    third_party_api_url = getAPIURL()+'sub_section_master/'+id
    ...
    data=json.loads(response.content)
    print("data",data)
    print("label",data['data']['sub_section_label'])
    section_details=data['data']['section_details']
    print("section_details",section_details)
    return render(request, 'add_sub_section.html',{'sub_section_label':data['data']['sub_section_label'],'section_details':section_details,
                                                     'sub_section_description':data['data']['sub_section_description'],'id':id})
