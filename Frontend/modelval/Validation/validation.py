...
class Validation:
    ...
    def assignValidation(self,validationType,assignTo,mdlId,addedBy,department_aid,originator_id):
        task_id_lst = []
        ...
        for colval in validationType:
            ...
            task_registration_obj=TaskRegistration(task_id=task_id,department=department,originator=originator,task_function=None,
    ...
    def getModelDocs(self,mdlId):
        ...
        print("check sql query----------",strQ) 
        tableResult =self.objdbops.getTable(strQ)
        print("check result----------",tableResult)
        tableResult= tableResult.to_json(orient='index')
        return json.loads(tableResult) 
